  ...
  group('WebShareService Tests', () {
    ...
    test('startServer binds and serves HTTP endpoints correctly', () async {
      ...
      try {
        ...
        final rootReq = await client.getUrl(
          Uri.parse('http://127.0.0.1:${service.port}/'),
        );
        ...
        final htmlContent = await rootRes
        ...
        final imgReq = await client.getUrl(
          Uri.parse('http://127.0.0.1:${service.port}/image?index=0'),
        );
        ...
        final imgBytes = await imgRes.fold<List<int>>(
          [],
        ...
        final img2Req = await client.getUrl(
          Uri.parse('http://127.0.0.1:${service.port}/image?index=1'),
        );
        ...
        final img2Bytes = await img2Res.fold<List<int>>(
          [],
          (prev, elem) => prev..addAll(elem),
        );
        ...
        final dlReq = await client.getUrl(
          Uri.parse('http://127.0.0.1:${service.port}/download?index=0'),
        );
        ...
        final dlBytes = await dlRes.fold<List<int>>(
          [],
          (prev, elem) => prev..addAll(elem),
        );
        ...
        expect(
          events.any(
            (e) =>
                e.type == WebShareEventType.fileDownloaded && e.fileIndex == 0,
          ),
          isTrue,
        );

        // 5. Test GET /download-all (ZIP archive)
        final zipReq = await client.getUrl(
          Uri.parse('http://127.0.0.1:${service.port}/download-all'),
        );
        ...
        final zipBytes = await zipRes.fold<List<int>>(
          [],
          (prev, elem) => prev..addAll(elem),
        );
        ...
        final notFoundReq = await client.getUrl(
          Uri.parse('http://127.0.0.1:${service.port}/random_route'),
        );
