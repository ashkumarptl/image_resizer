...
void main() {
  group('Low-End Device Performance & Isolate Dimension Tests', () {
    ...
    test('readImageDimensions returns correct dimensions without freezing main thread', () async {
      final jpegDims = await ImageProcessor.readImageDimensions(testJpegFile.path);
      expect(jpegDims, isNotNull);
      expect(jpegDims!.width, 240);
      expect(jpegDims.height, 160);

      final pngDims = await ImageProcessor.readImageDimensions(testPngFile.path);
      expect(pngDims, isNotNull);
      expect(pngDims!.width, 240);
      expect(pngDims.height, 160);

      // Non-existent file test
      final nullDims = await ImageProcessor.readImageDimensions('${tempDir.path}/does_not_exist.jpg');
      expect(nullDims, isNull);
    });

    test('processImage streams onProgress callbacks with stage descriptions', () async {
      ...
      expect(stageMessages.any((msg) => msg.toLowerCase().contains('reading')), true);
  ...
  group('ProcessingProgressModal Widget Tests', () {
    testWidgets('Renders progress percentage, stage description, and reassurance badge',
        (tester) async {
      ...
      expect(find.text('Optimizing resolution & compression...'), findsOneWidget);
      expect(find.text('Background Isolate • UI stays responsive'), findsOneWidget);
