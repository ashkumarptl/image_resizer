...
void main() {
  ...
  group('SystemIntegrationService Unit Tests', () {
    test('getInitialSharedFile invokes channel and returns file path', () async {
    ...
    test('getInitialShortcut invokes channel and returns shortcut id', () async {
      final service = SystemIntegrationService.instance;
    ...
    test('openInGallery invokes channel with correct path', () async {
      final service = SystemIntegrationService.instance;
      final success = await service.openInGallery('/storage/emulated/0/DCIM/test.jpg');

      expect(success, isTrue);
      final openCall = methodCalls.firstWhere((call) => call.method == 'openInGallery');
      expect(openCall.arguments, equals({'path': '/storage/emulated/0/DCIM/test.jpg'}));
    });
    ...
    test('listens to incoming broadcast callbacks from native', () async {
      ...
      final ByteData data = const StandardMethodCodec().encodeMethodCall(
        const MethodCall('onSharedFileReceived', '/storage/emulated/0/DCIM/photo.jpg'),
      );
      await TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .handlePlatformMessage('com.ashspark.image_resizer/system_integration', data, (_) {});
  ...
  group('ResultScreen - Open in Gallery Integration Tests', () {
    testWidgets('displays Save to Gallery, saves successfully, and opens in Gallery via intent', (WidgetTester tester) async {
      ...
      final testFile = File('${tempDir.path}/test_output.jpg')..writeAsBytesSync([1, 2, 3]);
      ...
      await tester.pumpWidget(
        ProviderScope(
          ...
          child: MaterialApp(
            home: ResultScreen(result: dummyResult),
          ),
        ),
      );
