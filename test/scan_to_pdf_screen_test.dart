...
void main() {
  ...
  testWidgets(
    'ScanToPdfScreen renders clean header, floating action pill, and empty state',
    (WidgetTester tester) async {
      await tester.pumpWidget(
        ProviderScope(
          overrides: [authServiceProvider.overrideWithValue(MockAuthService())],
          child: const MaterialApp(home: ScanToPdfScreen(isTab: true)),
        ),
      );
      ...
      expect(
        find.textContaining('Contrast Background', findRichText: true),
        findsOneWidget,
      );
  ...
  testWidgets(
    'ScanToPdfScreen project card renders cleanly on small screens without overflow',
    (WidgetTester tester) async {
          ...
          child: MaterialApp(
            home: MediaQuery(
              ...
              child: SizedBox(
                width: 360,
                height: 640,
                child: ScanToPdfScreen(isTab: true, initialProjects: [project]),
              ),
            ),
          ),
        ),
  ...
  testWidgets(
    'ScanToPdfScreen filter bottom sheet opens without Material assertion error',
    (WidgetTester tester) async {
      final project = ScanProject(
        id: 'doc1',
        name: 'Sample Doc',
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [authServiceProvider.overrideWithValue(MockAuthService())],
          child: MaterialApp(
            home: ScanToPdfScreen(isTab: true, initialProjects: [project]),
          ),
        ),
      );
      await tester.pumpAndSettle();
