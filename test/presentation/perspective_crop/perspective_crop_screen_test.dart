...
void main() {
  ...
  setUpAll(() async {
    ...
    dummyImageFile = File('${Directory.systemTemp.path}/test_perspective_screen_img.jpg');
  ...
  group('PerspectiveCropScreen Widget Tests', () {
    testWidgets('Renders AppBar, Canvas, Presets, Filters, and Apply Button', (tester) async {
      await tester.pumpWidget(buildTestWidget());
      await tester.pumpAndSettle();

      // Verify AppBar elements
      expect(find.text('Perspective Crop & Deskew'), findsOneWidget);
      expect(find.text('Drag 4 corners to align document boundaries'), findsOneWidget);
    ...
    testWidgets('Tapping presets and filters updates active selection', (tester) async {
      ...
      var canvas = tester.widget<PerspectiveCropCanvas>(find.byType(PerspectiveCropCanvas));
    ...
    testWidgets('Applying crop in returnCroppedFile mode pops with resulting file', (tester) async {
      File? returnedResult;
