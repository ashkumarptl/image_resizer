...
void main() {
  ...
  testWidgets('SendToPcSheet renders header, clean QR code, Copy IP button, and 3-step visual graphics', (tester) async {
    ...
    expect(find.textContaining('Cyber Cafe & Form Fillers Transfer'), findsOneWidget);
  ...
  testWidgets('ResultScreen renders prominent CYBER CAFE / PC badge on Send to PC button and helper hint', (tester) async {
    ...
    await tester.pumpWidget(
      ProviderScope(
        child: MaterialApp(
          home: ResultScreen(result: mockResult),
        ),
      ),
    );
    ...
    expect(
      find.textContaining('Filling a form on PC? Transfer wirelessly without WhatsApp or cable.'),
      findsOneWidget,
    );
  });
}
